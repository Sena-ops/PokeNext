...
import { Search, GitBranch, Home } from 'lucide-react';
...
export default function EvolutionIndexPage() {
  ...
  const { data: pokemonList, isLoading } = useQuery({
    queryKey: ['pokemon-list-evolution'],
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-4xl font-bold">Evolution Chains</h1>
        </div>
        <p className="text-gray-600 dark:text-gray-400">
          Selecione um Pokémon para ver sua cadeia evolutiva completa
        </p>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {filteredPokemon.map((pokemon, index) => (
            <Link key={pokemon.name} href={`/evolution/${pokemon.name}`}>
              <motion.div
